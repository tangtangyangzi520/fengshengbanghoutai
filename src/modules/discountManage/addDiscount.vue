            ...
            <div slot="content">
                <form  id='submitform1'>
                    ...
                    <div class="form-group">
                        ...
                        &nbsp;&nbsp;
                        <p style="margin-left:-1.7%;margin-top:-1.2%">
                            <span style="color:#999999">15个字以内</span>
                        </p>
                    </div>
                    <br>
                    <div class="form-group">
                    ...
                    <div class="form-group" >
                        ...
                        <button class="btn yellow-crusta" type="button" @click="showselect" style="margin-left:1.57%">添加</button>
                        <br><br>
                        <span style="margin-left:26.5%">批量减价 
                            <input type="number" class="input-sm-2" v-model="reduce" step="0.01" @keyup="integer($event)" @blur="integer($event)" min="0.00" max="9999999"/> 元  &nbsp;&nbsp;
                                ...
                                <tr>
                                    ...
                                    <th style="width:35%;text-align:left">
                                        ...
                                        <a  style="text-decoration:none;" title="预览商品" > 
                                            <h4 class="tt">
                                                <p style="color:#333333;margin-bottom:2.8%;font-weight:400;font-style:normal;font-family:'Arial Normal', 'Arial'">{{spu.spuName}}</p>
                                                <span style="color:#FF9900">¥{{spu.minSalePrice}}</span>
                                                <span style="color:#333333;font-size:14px;font-family:'Arial Normal', 'Arial'">&nbsp;&nbsp;&nbsp;&nbsp;库存:{{spu.totalStockNum}}</span>
                                            </h4>
                                        </a>
                                    </th>
                                    <th style="width:20%"> 减价 &nbsp;&nbsp;
                                        <input type="number" class="input-sm-2" @keyup="reduceBefore(spu.spuId,spu.minSalePrice,$event)" :id="'before'+spu.spuId" step="0.01" required="required" min="0.01" 
                                            @change="reduceBefore(spu.spuId,spu.minSalePrice,$event)" :max="spu.minSalePrice" @click="clickreduceBefore(spu.spuId,spu.minSalePrice,$event)" :value="spu.reduce"
                                            @blur="reduceBefore(spu.spuId,spu.minSalePrice,$event)"/>  元
                                    </th>
                                    <th style="width:18%"> 减价后
                                        <input type="number" class="input-sm-1" @keyup="refuceAfter(spu.spuId,spu.minSalePrice,$event)" :id="'after'+spu.spuId" step="0.01" required="required" 
                                            @change="refuceAfter(spu.spuId,spu.minSalePrice,$event)" min="0.01" :max="spu.minSalePrice" @blur="refuceAfter(spu.spuId,spu.minSalePrice,$event)"> 元
                                    </th>
