    ...
    <form class="row search-form" style="margin: 5px 0 0 0">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <div class="col-md-9">
                     <select class="form-control" v-model="searchOptions.orsClientType">
                        <option v-for="item in orderSourceList" :value="item.id">{{item.name}}</option>
                    </select>
                </div>
            </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    clearOptions() {
      ...
      //修改下拉框的值
      let list = this.stateList,
        listLogi = this.logiTypeList,
        listOrderType = this.orderTypeList;
      this.stateList = [];
      this.logiTypeList = [];
      this.orderTypeList = [];
