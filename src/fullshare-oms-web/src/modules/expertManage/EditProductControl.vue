                       ...
                       <div class="col-md-6 time-box">
                        <input type="text" class="form-control inline-block datePicker" placeholder="选择开始时间" id="createStartTime1" v-model="stime"/>
                                <select  v-model="hour">
                                    ...
                                    <option v-for="item in 23" :value="item">{{item}}时</option>
                                </select>&nbsp;&nbsp;
...
export default {
       ...
       /* if(this.resourceList.length == 0 ){
        ...
        //详情图片管理
         ...
         this.imgList.forEach((data,index)=>{
            ...
            client.postData("http://192.168.4.249:8086/eb-manager/spu/editProductSpu", this.request).then(data => {
                if (data.code == 200) {
                    this.showMsg("编辑成功")
                    this.expertEditId = '';
  ...
  "parentCode": "product_insurance"
    ...
    watch: {
        proflag(){
                   client.postData("http://192.168.4.249:8086/eb-manager/spu/getProductSpuById?spuId="+this.spuid, {}).then(data => {  //192.168.4.249
                this.isLoading = false;
                  if (data.code == 200) {
                    ...
                    $("#selects span").each(function(index,val){
                        ...
                        for(var i=0;i< data.data.piList.length;i++){
                          ...
                          if(data.data.piList[i].piInsurance +','+data.data.piList[i].piInsuranceId+','+data.data.piList[i].piSort == $($(val).children("input")).val()){
                                 ...
                                 $($(val).children("input")).attr("checked","true");  
                          }
                    }
                })
          ...
          //图片回显
            client.postData("http://192.168.4.249:8086/eb-manager/psr/getPsrBannerById?spuID="+this.spuid, {}).then(data => {  //192.168.4.249
                this.isLoading = false;
            ...
            //详情图片回显
            client.postData("http://192.168.4.249:8086/eb-manager/psr/getPsrDetailsById?spuID="+this.spuid, {}).then(data => {  //192.168.4.249
        ...
        shangb() {
            if(this.shangb.length > 3){
                this.showMsg("最多选择3个理由")
                 $($("#selects span").children("input")).attr("checked","false");  
                return
            }else{
