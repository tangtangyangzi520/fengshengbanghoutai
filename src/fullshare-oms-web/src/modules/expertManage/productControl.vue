        ...
        <m-alert v-if="!removeAddDialog" :title="title" :hide-btn="true" :show="showDialog" :onhide="hideDialog" :onsure="submitInfo" :effect="'fade'" :width="'1000px'" >
                ...
                <div class="row" style="background-color:#F0F0F0">
                    <form class="form-horizontal" name="addForm" role="form">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-4">
                                <select class="form-control" v-model="shangb" multiple="multiple">
         ...
         <hr style="height:1px;border:none;border-top:1px solid white;" /><br>    
                     ...
                     <div   v-for="(v,i) in sxlist"  style ="width: 100%; background-color:white;
                        ...
                        <div class="baseSx">                     
                         <span class="pca" style="margin-left:10%">{{i.pcaName}}
                         ...
                         <span v-for="s in i.pcaoList" class="pcao">                      
                                  <input type="checkbox" class="input" :value="s.pcaoName" > {{s.pcaoName}}
                            ...
                            <span v-for="a in 10">&nbsp;</span>
                         <span class="addsxxx" >
                            <input type="checkbox" class="input tianjia" value="1" > <input type="text" style ="width: 5%; height:100%"  placeholder="">
                            <a class="" @click="addsxxx($event)" >添加</a>  &nbsp;&nbsp;没有合适的？可以自己输入。 
                        <span>
                             <a class="delete" @click="closeProperty($event)" >×</a>   
                        </div>
                    </div>
 <br><font style="color:black" size="3">还可以添加 2 组，销售属性总数最多4组，SKU组合不能超过600组。</font>
                        <div class="form-group">
                            <label for="title" class="col-sm-3 control-label">
             ...
             <hr style="height:1px;border:none;border-top:1px solid white;" /><br>  
                     ...
                     <font style="color:#6699CC" size="3">只有一个SKU的情况：如需添加销售属性请先生成SKU组合</font>
                        <div class="form-group" >
                            <label for="title" class="col-sm-3 control-label">
                                <span class="required">* </span>丰盛榜售价：
                            </label>
                            <div class="controls col-md-2">
                               <input type="text" class="form-control input-sm" v-model="sj" placeholder="">
                        </div>
                            <label for="title" class="col-sm-2 control-label">
                                <span class="required">* </span>原价：
                            </label>
                            <div class="controls col-md-2">
                               <input type="text" class="form-control input-sm" v-model="yj" placeholder="">
                            </div>
                    </div>
                         <div class="form-group">  
                            <label for="title" class="col-sm-3 control-label">
                            ...
                            <div class="controls col-md-2">
                               <input type="text" class="form-control input-sm" v-model="kc" placeholder="">
                            </div>
                            <label for="title" class="col-sm-2 control-label">
                                <span class="required">* </span>ERP库存（件）：
                            </label>
                            <div class="controls col-md-2">
                               <input type="text" class="form-control input-sm" v-model="erpkc" placeholder="">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="title" class="col-sm-3 control-label">
                          ...
                          <tr>
                             ...
                             <tbody id="itemList" >
                                <tr v-for="(index,group) in xssxList">
                                   <td style="width:30%"> <a class="dele" @click="close($event,index)" >×</a> {{ group }} <input class="input2" type="hidden" :value="group"/></td>
    ...
    <br><hr style="height:1px;border:none;border-top:1px solid white;" /><br>
                         ...
                         <div class="form-group">
                            ...
                            <div class="controls col-md-4" >
                               <span  class="insu" v-for="data in insurancelist">
                                <input type="checkbox"   :value="data.keyValue+','+data.dictionnaryId+','+data.sortNo" >&nbsp;&nbsp;{{ data.keyValue }}<br>
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="controls col-md-4 ">
                                <span class="radios">
                                <input type="radio" name="startTime"  value="1" checked>立刻                                                              
                                <br>
                                <input type="radio"  name="startTime"   value="2">设定<br>  
                                </span>
                       <div class="col-md-6 time-box">
                        <input type="text" class="form-control inline-block datePicker" placeholder="选择开始时间" v-model="time" id="createStartTime"/>
                                ...
                                <select v-model="minutes" >
                                    ...
                                    <option v-for="item in 60" :value="item+1">{{item+1}}分</option>
                                </select>
                        </div>  
                    </div>
                </div>                                                    
...
<!-- 
        ...
        <!-- 标签选择弹窗 -->
         ...
         <tag-tree v-if="showperTreeSelect" :list="personList" :onselect="selectPerFunc" :oncancel="showperDialog"></tag-tree>
        <tag-tree v-if="showneiTreeSelect" :list="neirongList" :onselect="selectNeiFunc" :oncancel="showneiDialog"></tag-tree>
...
export default {
    ...
    methods: {
        ...
        addsxxx( event ) {
            ...
            let f =  $(el).parent()
            f.append("<input type='checkbox' class='input' value='1' > <input type='text' style ='width: 5%; height:100%''  placeholder=''> ")
          ...
          if( this.yj == '' ){
           ...
           // alert($($($(".addsxxx")[d]).children("input:checked")).length)
                      ...
                      if( $($(zc).children("span[class='pcao']").children("input:checked")).length +  $($($(".addsxxx")[d]).children("input:checked")).length > 1 ){
                                   msg = $($(bname[d]).children("input:hidden")[0]).val()+":不是单一的sku,请先生成SKU组合"
                                   return
                            ...
                            let list =  $(zc).children("span[class='pcao']").children("input:checked").siblings("input:hidden")
                             let basesx = { //基本属性对象
                               ...
                               if( $.trim($($($(".addsxxx")[d]).children("input:checked").next(":text")).val() ) == "" ){
                                       //alert(1)
                                       msg = $($(bname[d]).children("input:hidden")[0]).val()+":添加属性值不能为空"
                                       return false
                               }
                             let newbase =  {
...
//alert(  xssx[a] )
           if( a < $(item).length -2){
                             ...
                             if($(zf).val() == zhi[f+len] ){
                                //匹配成功
                                 ...
                                 let map = {
                                          "optionList": [  //销售属性选项  
                                            {
                                              "pcaoName": zhi[f+len],  //  选项值
                                              "pcaoSortNo": g,       // 排序号
                                              "pcaoValue": zhi[f+len], //  选项值
                                             
                                              "skuOptionList": [    //sku属性选项
                                                {
                                                  "psaoAtrrValue": zhi[f+len], // 选项值  
                                                  "psaoSortNo": g,//排序号
                                                  "psaoType": 2  // 2
                                                }
                                              ]
            ...
            /*                     "pcaoList": [
              ...
              sin = {   
                        ...
                        "skuOptionList":baseList,
                        "attrList": attrList,
                        "skuAtrr": $(el[0]).val() ,          // 拼接属性
                        "skuClassify": "i",     //   i
                        "skuCode": $(el[5]).val(),        //   sku编码
                        "skuMarketSalePrice": $(el[2]).val(),   //   原价
                        "skuName":  $(el[0]).val()+spuName  ,      //    拼接属性spu名字
                        "skuNew": 1,   
                        "skuOverseas": 1,              // 1
                        "skuSalePrice": $(el[1]).val(),            // 售价   
                        "skuStockNum": $(el[4]).val(),            //  库存 $(el[4]).val()
                        "skuUnit": "",         //  "" 
                        "skuUrl": "",          //  ""
                        "skuWhId": "",                //   ""
                        "sku_remark": $(el[0]).val() ,     //  拼接属性
                        "skuShowNum": $(el[3]).val(),
    ...
    //保消类型
      ...
      //
        ...
        this.shangb.forEach((data,index)=>{
            ...
            client.postData("http://192.168.4.249:8086/eb-manager/spu/createProductSpu", this.request).then(data => {
                if (data.code == 200) {
                    //alert("新增成功")
                    //this.showMsg("新增成功")
        ...
        //获取消保类型
        getxbList() { 
        client.postData("http://192.168.4.249:8086/eb-manager/system/listDictionary", {
  "key": "",
  "keyValue": "",
  "page": {
    "currentPage": 0,
  ...
  "parentCode": "product_insurance"
        ...
        //获取上榜 理由
        getshangbangList() { 
        client.postData("http://192.168.4.249:8086/eb-manager/reason/getProductChartReasons", {}).then(data => {
                if (data.code == 200) {
        ...
        //获取品牌
        getbrandList() { 
        client.postData("http://192.168.4.249:8086/eb-manager/pbd/list", {"keyword": "string",
       "page": {
       "currentPage": 0,
       "pageSize": 0,
       "startIndex": 0,
    ...
    watch: {
        ...
        cflag() {
           ...
           client.postData("http://192.168.4.249:8086/eb-manager/pca/getProductCategoryAtrrPcraCatIds?parentIds="+this.spuCatId, {}  ).then(data => {
                if (data.code == 200) {
